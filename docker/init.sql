...
CREATE TABLE IF NOT EXISTS sales_order (
    ...
    sales_account_id BIGINT         DEFAULT NULL COMMENT '销售账户ID',
    ...
    recipient_phone VARCHAR(20)     DEFAULT NULL COMMENT '收件人电话（快照）',
    recipient_address TEXT          DEFAULT NULL COMMENT '收件地址（快照）',
    payment_channel_type_id BIGINT  DEFAULT NULL COMMENT '收款渠道ID',
    payment_amount  DECIMAL(12,2)   NOT NULL DEFAULT 0.00 COMMENT '收款金额',
    remark          TEXT            DEFAULT NULL COMMENT '备注',
    ...
    KEY idx_updated_by (updated_by),
    KEY idx_payment_channel_type_id (payment_channel_type_id),
    CONSTRAINT fk_order_customer FOREIGN KEY (customer_id) REFERENCES customer(id),
    CONSTRAINT fk_order_sales FOREIGN KEY (sales_person_id) REFERENCES sys_user(id),
    CONSTRAINT fk_order_sales_account FOREIGN KEY (sales_account_id) REFERENCES sales_account(id),
    CONSTRAINT fk_order_shipping_address FOREIGN KEY (shipping_address_id) REFERENCES customer_shipping_address(id),
    CONSTRAINT fk_order_payment_channel_type FOREIGN KEY (payment_channel_type_id) REFERENCES payment_channel_type(id),
    CONSTRAINT fk_order_updated_by FOREIGN KEY (updated_by) REFERENCES sys_user(id)
...
CREATE TABLE IF NOT EXISTS customer_shipping_address (
    ...
    is_default      TINYINT         NOT NULL DEFAULT 0 COMMENT '0=非默认 1=默认',
    status          TINYINT         NOT NULL DEFAULT 1 COMMENT '0=停用 1=启用',
    default_marker  BIGINT          GENERATED ALWAYS AS (IF(is_default = 1, customer_id, NULL)) VIRTUAL COMMENT '默认地址唯一标记',
    created_at      DATETIME        NOT NULL DEFAULT CURRENT_TIMESTAMP,
    updated_at      DATETIME        NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
    PRIMARY KEY (id),
    KEY idx_customer_id (customer_id),
    UNIQUE KEY uk_customer_default_address (default_marker),
...
CREATE TABLE IF NOT EXISTS sales_order_payment (
    ...
    KEY idx_order_id (order_id),
    KEY idx_payment_channel_type_id (payment_channel_type_id),
    CONSTRAINT fk_sop_order FOREIGN KEY (order_id) REFERENCES sales_order(id) ON DELETE CASCADE,
...
CREATE TABLE IF NOT EXISTS order_tracking (
    ...
    UNIQUE KEY uk_order_tracking (order_id, tracking_no),
...
CREATE TABLE IF NOT EXISTS order_tracking_item (
    ...
    CONSTRAINT fk_tracking_item FOREIGN KEY (tracking_id) REFERENCES order_tracking(id) ON DELETE CASCADE
...
CREATE TABLE IF NOT EXISTS customer_audit_log (
    ...
    operated_at     DATETIME        NOT NULL DEFAULT CURRENT_TIMESTAMP COMMENT '操作时间',
    PRIMARY KEY (id),
    KEY idx_caudit_customer (customer_id),
    KEY idx_caudit_operator (operator_id),
    CONSTRAINT fk_caudit_customer FOREIGN KEY (customer_id) REFERENCES customer(id) ON DELETE CASCADE,
    CONSTRAINT fk_caudit_operator FOREIGN KEY (operator_id) REFERENCES sys_user(id)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COMMENT='顾客修改日志';
