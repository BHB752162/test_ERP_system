  ...
  <div>
    <el-card shadow="never" class="search-card">
      <el-form :model="query" inline>
        <el-form-item label="客户名称">
          <el-input v-model="query.keyword" placeholder="名称/电话" clearable @keyup.enter="search" />
        </el-form-item>
        <el-form-item>
    ...
    <el-card shadow="never" style="margin-top: 16px">
      ...
      <el-table :data="list" border stripe v-loading="loading">
        <template #empty>
          <el-empty :description="error || (query.keyword ? '没有找到匹配的客户' : '暂无客户数据')" />
        </template>
...
const { list, total, loading, error, query, fetchData, search, reset, onPageChange } = useCrudList(listCustomers, { defaultQuery: { keyword: '' } })
